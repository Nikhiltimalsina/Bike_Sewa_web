...
import { getBlogByIdApi, updateBlogApi } from "@/api/blog.api";
import { z } from "zod";
...
export default function EditBlogPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const loadBlog = async () => {
      try {
        ...
        setSubmitMessage({
          type: "error",
          text: error instanceof Error ? error.message : "Failed to load blog",
        });
      } finally {
        setIsFetching(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSubmitMessage({ type: "success", text: "Blog updated successfully!" });
  ...
  if (isFetching) {
    return (
      <div className="p-6 flex items-center justify-center min-h-screen bg-[#161b27]">
